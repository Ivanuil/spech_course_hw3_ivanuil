...
class AngularMarginSoftmax(torch.nn.Module):
    ...
    def __init__(self, embedding_dim: int, num_classes: int, margin: float, scale: float):
        ...
        self.num_classes = num_classes # количество аудиофайлов
        self.margin = margin
        self.scale = scale
    ...
    def forward(self, embeddings: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        ...
        # косинусное расстояние между весами и эмбеддингами
        cosine = F.linear(normalized_embeddings, normalized_weight)
        # указываем разброс значений
        cosine = cosine.clamp(-1 + self.eps, 1 - self.eps)
        sine = torch.sqrt(1.0 - torch.pow(cosine, 2))
        cosine_theta_m = cosine * math.cos(self.margin) - sine * math.sin(self.margin)
        one_hot = torch.zeros_like(cosine)
        one_hot.scatter_(1, labels.view(-1, 1).long(), 1)
        output = (one_hot * cosine_theta_m) + ((1.0 - one_hot) * cosine)
        output *= self.scale
    ...
    def predict(self, embeddings: torch.Tensor) -> torch.Tensor:
        ...
        # расчет косинусного сходства и предсказание класса с наибольшим сходством
        cosine_similarity = F.linear(normalized_embeddings, normalized_weight)
        predictions = torch.argmax(cosine_similarity, dim=1)
...
class ModuleParams:
    ...
    loss_function: str = "angular_margin"  # "cross_entropy" or "angular_margin"
    validation_dir: Path | None = None
    validation_frequency: int = 3
...
def evaluate(
    ...
    genuine_mask = (labels_matrix == 1) & mask
    impostor_mask = (labels_matrix == 0) & mask
    
    genuine_scores = similarity_matrix[genuine_mask]
    impostor_scores = similarity_matrix[impostor_mask]
    
    max_samples = 10000
    if len(genuine_scores) > max_samples:
        indices = torch.randperm(len(genuine_scores))[:max_samples]
        genuine_scores = genuine_scores[indices]
    
    if len(impostor_scores) > max_samples:
        indices = torch.randperm(len(impostor_scores))[:max_samples]
        impostor_scores = impostor_scores[indices]
    
    # Calculate EER
    thresholds = torch.linspace(0.0, 1.0, 1000)
    min_diff = float('inf')
    eer = 0.5
    ...
    print(f"EER: {eer:.4f} (Genuine: {len(genuine_scores)}, Impostor: {len(impostor_scores)})")
    return float(eer)
...
def main(conf: ModuleParams) -> None:
    ...
    for epoch in pbar:
        ...
        # Сохранение стандартного checkpoint
        torch.save(
            {
                "epoch": epoch + 1,
        ...
        # Дополнительное сохранение в формате PyTorch Lightning checkpoint
        lightning_checkpoint = {
            'epoch': epoch + 1,
            'global_step': global_step,
            'pytorch-lightning_version': '1.9.0',  # Укажите актуальную версию
            'state_dict': model.state_dict(),
            'optimizer_states': [optim.state_dict()],
            'lr_schedulers': [],
            'callbacks': {},
            'hparams': {
                'model_params': model_params,
                'loss_function': conf.loss_function,
                'angular_margin': conf.angular_margin if conf.loss_function == "angular_margin" else None,
                'angular_scale': conf.angular_scale if conf.loss_function == "angular_margin" else None,
                'learning_rate': conf.learning_rate,
                'batch_size': conf.batch_size,
            },
            'current_score': epoch_accuracy,
            'best_model_score': epoch_accuracy,
            'best_model_path': str(conf.checkpoints_dir / f"epoch_{epoch + 1}.ckpt"),
        }
        
        torch.save(
            lightning_checkpoint,
            conf.checkpoints_dir / f"lightning_epoch_{epoch + 1}.ckpt"
        )
    ...
    torch.save(
        final_checkpoint,
        conf.checkpoints_dir / "final_model.ckpt"
    )
